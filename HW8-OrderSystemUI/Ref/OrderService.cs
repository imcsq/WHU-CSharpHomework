...
namespace HW5_OrderSystem2._0
{
    ...
    class OrderService
    {
        public List<Order> orders = new List<Order>(); //订单集
        ...
        // XML序列化所有订单
        public void ExportAllOrdersToXML(string filename)
        {
            ...
            {
                Console.WriteLine("导出为XML文件时出错。出错信息：" + e);
            }
        }

        // 从XML序列化文件恢复订单
        public void ImportAllOrdersToXML(string filename)
        {
            XmlSerializer xmlser = new XmlSerializer(orders.GetType());
            try
            {
